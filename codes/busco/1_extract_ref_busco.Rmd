...
# iterate over BUSCO
df_busco_summary <- foreach (gene = ls_busco, .combine='rbind') %dopar% {
  require(stringr)
  ...
  seq1 <- Biostrings::readDNAStringSet(paste0(currentdir, "/filtered/step1_", gene, ".fna"))
  seq1_sp <- length(seq1)
  seq1_min_len <- min(stringr::str_count(as.character(seq1)))
  seq1_max_len <- max(stringr::str_count(as.character(seq1)))

  seq1_aligned <- Biostrings::readDNAStringSet(paste0(currentdir, "/filtered/step1_", gene, "_aligned.fna"))
