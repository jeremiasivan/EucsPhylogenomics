...
if (!is.null(ls_target_species)) {
    ...
    # extract sister taxa
    sister_list <- lapply(alltre, f_get_sister_taxa, focal_sp=params$focal_species)
    sister_list <- sister_list[!sapply(sister_list, is.na)]

    # extract frequency
    sister_ids <- sapply(sister_list, function(x) {
        if (!all(is.na(x))) {
    ...
    # count the frequency
    df_count <- as.data.frame(table(sister_ids))
    colnames(df_count) <- c("species_two", "n")

    df_count <- df_count %>% mutate(species_one=params$focal_species, .before=species_two) %>% arrange(desc(n))
}
