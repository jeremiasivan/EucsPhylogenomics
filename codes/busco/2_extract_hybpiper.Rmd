...
params:
  ...
  fn_metadata: "~/EucsPhylogenomics/files/hybridisation_metadata.tsv"
...
for (busco in ls_target_busco) {
    ...
    names(busco_seq) <- sapply(names(busco_seq), function(x) {
        # ls_header <- unlist(strsplit(x, split="_"))
        #
        # header <- paste(ls_header[1:2], collapse="_")
        # if (header=="E_sid") {
        #     header <- "E_sid_E_mel"
        # }
        # header <- gsub(paste0("-", busco, "$"), "", x)
        # header <- paste0(header, "_REF")

        header <- gsub(paste0("-", busco, "$"), "_REF", x)
    })
...
fn_concat <- paste0(params$outdir, "/concat.fa")
fn_partition <- paste0(params$outdir, "/partition.nex")
fn_sptree <- paste0(params$outdir, "/fasttree.tre")

# combine all treefiles
f_amas_dna(paste0(params$outdir, "/*/post_mafft/*_final.fa"), fn_concat, fn_partition, params$exe_amas)
system(paste("sed 's/\?/-/g'", fn_concat))

# run FastTree
f_fasttree(fn_concat, fn_sptree, params$exe_fasttree)
```

```{r quibl}
# read alltree
alltrees <- ape::read.tree(fn_all_treefile)
sptree <- ape::read.tree(fn_sptree)
...
# read QuIBL metadata
df_quibl_metadata <- data.table::fread(params$fn_metadata)
...
df_output_quibl <- foreach (i = 1:nrow(df_quibl_metadata), .combine='rbind') %dopar% {
    ...
    ls_subset_tre_transformed <- lapply(ls_subset_tre, function(x) {
        total_bl <- sum(x$edge.length)
        x$edge.length <- x$edge.length / total_bl
        return(x)
    })
    ...
    # extract topology proportion
    ls_subset_top <- sapply(ls_subset_tre, function(x) {
        x$node.label <- NULL
        x$edge.length <- NULL
        x <- ape::root(x, outgroup=df_quibl_metadata$outgroup[i])
        x <- ape::drop.tip(x, tip=df_quibl_metadata$outgroup[i])
        ape::write.tree(x)
    })

    # match the topology names
    df_subset_top <- data.table::data.table()
    uq_subset_top1 <- unique(ls_subset_top)
    uq_subset_top2 <- unique(ls_subset_top)

    for (j in uq_subset_top1) {
        for (k in uq_subset_top2) {
            if (j==k) {
                df_subset_top <- rbind(df_subset_top, data.table::data.table(top1=j, top2=k))
                uq_subset_top2 <- uq_subset_top2[uq_subset_top2!=k]
                next
            }

            if (ape::all.equal.phylo(ape::read.tree(text = j), ape::read.tree(text = k))) {
                df_subset_top <- rbind(df_subset_top, data.table::data.table(top1=j, top2=k))
                uq_subset_top1 <- uq_subset_top1[uq_subset_top1!=k]
                uq_subset_top2 <- uq_subset_top2[uq_subset_top2!=k]
            }
        }
    }

    for (j in 1:nrow(df_subset_top)) {
        ls_subset_top[ls_subset_top == df_subset_top$top2[j]] <- df_subset_top$top1[j]
    }

    # count the proportion of each topology
    df_top_prop <- data.frame(top=ls_subset_top, freq=1) %>%
                        group_by(top) %>%
                        summarise_all(sum) %>%
                        arrange(desc(freq)) %>%
                        mutate(top=forcats::fct_inorder(top))
    df_top_prop$top <- gsub(df_quibl_metadata$species_one[i], "1", df_top_prop$top)
    df_top_prop$top <- gsub(df_quibl_metadata$species_two[i], "2", df_top_prop$top)
    df_top_prop$top <- gsub(df_quibl_metadata$species_three[i], "3", df_top_prop$top)
    data.table::fwrite(df_top_prop, file=paste0(temp_outdir_quibl, "topology_count.tsv"), sep="\t", quote=F)
    ...
    if (file.exists(paste0(temp_outdir_quibl, "quibl.csv"))) {
        ...
        quibl_mixprop2_count <- quibl_mixprop2*quibl_count
        quibl_count <- sum(df_quibl$count)
        quibl_mixprop2 <- quibl_mixprop2_count/quibl_count
    }
    ...
    if (file.exists(paste0(temp_outdir_quibl, "quibl_transformed.csv"))) {
        ...
        quibl_transformed_mixprop2_count <- quibl_transformed_mixprop2*quibl_transformed_count
        quibl_transformed_count <- sum(df_quibl_transformed$count)
        quibl_transformed_mixprop2 <- quibl_transformed_mixprop2_count/quibl_transformed_count
    }
    ...
    return(data.table::data.table(species_one=df_quibl_metadata$species_one[i],
                                  ...
                                  sptree_pairwise_dist=sptree_distance[rownames(sptree_distance)==df_quibl_metadata$species_one[i],
...
```{r pixy}
# output directory
dir_output_pixy <- paste0(params$outdir, "/pixy/")

# read QuIBL metadata
df_pixy_metadata <- data.table::fread(params$fn_metadata)

# convert partition file to BED format
fn_partition_bed <- paste0(params$outdir, "/partition.bed")
f_nexus2bed(fn_partition, fn_partition_bed)
...
# iterate over quartet
foreach (i = 1:nrow(df_pixy_metadata)) %dopar% {
    # output directory
    temp_outdir_pixy <- paste0(dir_output_pixy, df_pixy_metadata$species_two[i], "/")
    if (!dir.exists(temp_outdir_pixy)) {
        dir.create(temp_outdir_pixy, recursive=T)
    }

    # input/output files
    fn_species_list <- paste0(temp_outdir_pixy, "species_list.txt")
    fn_population <- paste0(temp_outdir_pixy, "population.txt")
    fn_concat_subset <- paste0(temp_outdir_pixy, "concat.fa")
    fn_vcf_subset <- paste0(temp_outdir_pixy, "concat.vcf")
    fn_vcf_subset_gz <- paste0(temp_outdir_pixy, "concat.vcf.gz")

    # save the species list
    ls_species <- c(df_pixy_metadata$species_one[i],
                    df_pixy_metadata$species_two[i],
                    df_pixy_metadata$species_three[i],
                    df_pixy_metadata$outgroup[i])
    writeLines(ls_species, con=fn_species_list)

    # save the population
    ls_population <- sapply(ls_species, function(x) {
        paste(unlist(strsplit(x, split="_"))[1:2], collapse="_")
    })
    df_population <- data.table::data.table(species=ls_species, population=ls_population)
    data.table::fwrite(df_population, file=fn_population, sep="\t", quote=F, col.names=F, row.names=F)

    # extract species from concatenated MSA
    cmd_seqkit_grep <- paste(params$exe_seqkit, "grep -n -f", fn_species_list, fn_concat, ">", fn_concat_subset)
    system(cmd_seqkit_grep)

    # convert MSA to VCF
    cmd_snpsites <- paste(params$exe_snpsites, "-v -o", fn_vcf_subset, fn_concat_subset)
    system(cmd_snpsites)

    # compress the VCF
    system(paste("bgzip", fn_vcf_subset))
    system(paste("tabix", fn_vcf_subset_gz))

    # run pixy
    cmd_pixy <- paste(params$exe_pixy, "--stats pi dxy",
                      "--vcf", fn_vcf_subset_gz,
                      "--populations", fn_population,
                      "--bed_file", fn_partition_bed,
                      "--output_folder", temp_outdir_pixy,
